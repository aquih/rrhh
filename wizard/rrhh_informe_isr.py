...
class rrhh_informe_isr(models.TransientModel):
    ...
    def print_report_excel(self):
        for w in self:
            ...
            hoja.write(5, 13, "Aguinaldo", bold)
            hoja.write(5, 14, "Bono 14", bold)
            hoja.write(5, 15, "Cuota IGSS", bold)
            hoja.write(5, 16, "Ingresos", bold)
            ...
            if lote_id.slip_ids:
                ...
                for nomina in lote_id.slip_ids:
                    ...
                    hoja.write(fila, 13, calculo_isr["aguinaldo_mes"])
                    hoja.write(fila, 14, calculo_isr["bonoc_mes"])
                    hoja.write(fila, 15, calculo_isr["cuota_igss"])
                    hoja.write(fila, 16, calculo_isr["rubro_ingresos"])
