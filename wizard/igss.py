...
class rrhh_igss_wizard(models.TransientModel):
    ...
    def generar(self):
        datos = ''
        for w in self:
            datos += str(w.payslip_run_id.slip_ids[0].company_id.version_mensaje) + '|' + str(datetime.today().strftime('%d/%m/%Y')) + '|' + str(w.payslip_run_id.slip_ids[0].company_id.numero_patronal) + '|'+ str(w.payslip_run_id.date_start.strftime('%m')).lstrip('0')
            datos += '|' + str(w.payslip_run_id.date_start.strftime('%Y')).lstrip('0') + '|' + str(w.payslip_run_id.slip_ids[0].company_id.name) + '|' +str(w.payslip_run_id.slip_ids[0].company_id.vat) + '|'+ str(w.payslip_run_id.slip_ids[0].company_id.email) + '|' + str(w.payslip_run_id.slip_ids[0].company_id.tipo_planilla) + '\r\n'
            datos += '[centros]' + '\r\n'
            ...
            for slip in w.payslip_run_id.slip_ids:
                fecha_planilla = w.payslip_run_id.date_start
                ...
                if contrato_ids:
                    ...
                    if contrato.date_end:
                        mes_contrato= contrato.date_end
                        ...
                        if mes_planilla == mes_final_contrato and anio_final_contrato == anio_planilla:
                            datos += str(contrato.wage) + '|' + str(contrato.date_start) + '|' + str(contrato.date_end) + '|'
                    else:
                        mes_contrato = contrato.date_start
                        mes_final_contrato = mes_contrato.month
                        anio_final_contrato = mes_contrato.year
                        if mes_final_contrato == mes_planilla and anio_final_contrato == anio_planilla:
                            datos += str(contrato.wage) + '|' + str(contrato.date_start) + '|' + '|'
                        else:
